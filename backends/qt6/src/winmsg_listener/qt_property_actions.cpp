...
QJsonObject handleGetProperty(QtObjectStore& store, int id, const QString& name) {
    ...
    // TODO: add GetProperties to list available properties.
...
QJsonObject handleSetProperty(QtObjectStore& store, int id, const QString& name, const QJsonValue& value) {
    ...
    if (propIndex >= 0) {
        ...
        const QMetaType targetType = prop.metaType();
        if (converted.metaType() != targetType && converted.canConvert(targetType))
            converted.convert(targetType);
